...
// If DATABASE_URL isn't provided (local dev without DB), avoid constructing
// PrismaClient (which can throw with some engine configs). Provide a
// runtime-friendly fallback proxy so importing modules won't crash the server.
...
if (process.env.DATABASE_URL) {
	try {
		...
		// Keep server alive — log the error and fall back to a proxy that
		// throws a helpful message when any DB call is attempted.
